...
func Exit(code int) {
	Syscall(SYS_UCORE_EXIT_GROUP, uintptr(code), 0, 0)
	return
}
...
func Kill(pid int, sig int) (errno int) {
	_, _, e1 := Syscall(SYS_UCORE_KILL, uintptr(pid), uintptr(sig), 0)
	errno = int(e1)
	return
}
...
func UcoreSleep(nsec int64) (errno int) {
	var sec uintptr;
	sec = uintptr(nsec / 10000000);			// 10 ms a unit;
	_, _, e1 := Syscall(SYS_UCORE_SLEEP, sec, 0, 0);
	errno = int(e1)
	return
}
...
func Write(fd int, p []byte) (n int, errno int) {
	for i := 0; i < len(p); i++ {
		Syscall(SYS_UCORE_PUTC, uintptr(p[i]), 0, 0)
	}
	n = len(p);
	return
}
...
func Gettimeofday(tv *Timeval) (errno int) {
	_, _, e1 := Syscall(SYS_UCORE_GETTIMEOFDAY, uintptr(unsafe.Pointer(tv)), 0, 0)
	errno = int(e1)
	return
}
